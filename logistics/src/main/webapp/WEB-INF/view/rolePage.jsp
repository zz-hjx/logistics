...
function operationFormatter(value,row,index){
	 var html = "";
	 html +="<span onclick='role_edit("+value+");' style='color: green;cursor: pointer;' class='glyphicon glyphicon-pencil'></span>&nbsp;&nbsp;&nbsp;";
	 html +="<span onclick='role_del("+value+");' style='color: red;cursor: pointer;' class='glyphicon glyphicon-trash'></span>";
...
/*角色-增加*/
function role_add(){
	layer_show("新增角色","role/edit.do",800,500);
}
...
function role_edit(roleId){
	layer.open({
	  ...
	  maxmin :true,
	  area: ['800px', '500px'],
	  content: 'role/edit.do?roleId='+roleId
	});
}
...
function role_del(roleId){
	layer.confirm('确认要删除吗？',{icon: 3},function(index){
		...
		//使用ajax 执行删除操作
		//$.get("admin/delete.do",{roleId:roleId},function(data){})
